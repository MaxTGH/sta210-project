...
How do different disaster characteristics (such as time of year, type, and total casualties incurred) predict the total economic damage caused by natural disasters in USD?
...
The dataset includes disasters that occurred from 2000 to 2025. Each observation is a different disaster that occurred around the world. Of each disaster, data on the number of people injured, affected, homeless, or killed, the total economic damages, type of disaster, exact location, start date and end date of the disaster were collected.

The data dictionary can be found [here](https://github.com/sta210-sp26/project-MaxTGH-hyeonsuan06/blob/main/data/README.md).

## 1.6 Data Cleaning

To clean the raw natural disaster dataset, the dataset was first reduced to only the variables relevant to the analysis. These included information on disaster type, location (country), event timing (start and end year, month, and day), and key impact measures such as total deaths, number injured, number affected, total affected, and total economic damage (in thousands of USD). Rows containing missing values were then removed using drop_na(). We attempted to make sure there was no relationship between the observations with missing values. Using the event timing data, a new variable duration_days was calculated as the difference between the end date and start date. An additional day was added to this value to ensure that the duration includes the start date.
...
**Response variable** - log-transformed total economic damage (total economic damage in thousands of USD)
...
events |>
  ...
  geom_histogram() + 
  labs(
    x = "Log Economic Losses (thousands, USD)",
...
2.  duration - duration in days of the disaster (quantitative)
    ...
    There seems to be a weak positive linear relationship between the log of duration in days with log of total economic damage (in thousands of dollars). The r-squared value is 0.0036. There is a lot of variability in the response variable when the log of duration in days is low, but the variability decreases as the x value increases. There are outliers present throughout the graph.
    ...
    events |>
      ...
      labs(
        x = "Log of Duration in Days",
        y = "Log Economic Losses (Thousands, USD)",
        title = "Relationship between Duration of Natural Disaster and Total Economic Damage",
...
res1 <- collect_metrics(events_fit_rs1) |>
...
res2 <- collect_metrics(events_fit_rs2) |>
  select(.metric, mean, n, std_err)
...
bind_rows(res1, res2) |>
  mutate(
    model = c(rep("Model 1", 2), rep("Model 3", 2))
  ) |>
  relocate(model) |>
  kable (digits = 3)
```

After evaluating the performance of the two models through cross-validation, we discovered that the values for RMSE and r-squared were similar again for model 1 and model 3. Because model 3 is more parsimonious, we decided to choose the model without log(no_affected) as our final model.
...
For the chosen model, the model diagnostics, including the linearity, equal variance, normality, independence, and multicollinearity (more detailed information/analysis found in the appendix under model diagnostics), were all satisfied and there were no influential points in the data set.
...
The final model has an RMSE of 2.592 on the test dataset, meaning the predicted values of log total economic damage (in thousands of USD) deviate from the observed values by approximately 2.592, on average. Given the IQR is 3.332 but the range is 16.678 for the log total economic losses (thousands, USD), we conclude that this model performs decently well. **Transforming back to the original scale, the predicted values for total economic damage deviate from the observed values by approximately \$13,356**.

The statistically significant predictors (significance level of 0.10) in the model provide meaningful insight into what drives economic damage from natural disasters:
...
4.  disaster_typeWildfire:
    ...
    Wildfires are expected to multiply total economic damage by a factor of 3.251 (e\^1.179 = 3.251) compared to earthquakes (the baseline category), holding all other variables constant.

The coefficients from the model suggest that human impact measures, particularly the number of people injured and disaster duration, are the most important observable predictors of economic damage from natural disasters. The positive relationship between injuries and economic damage makes intuitive sense: a higher injury toll signals a more intense and widespread event, which simultaneously destroys physical infrastructure and strains public health systems, all of which translate into greater economic costs.

# 4 Discussion

The most prominent finding from our project is that human impact measures (specifically the log of the number of people injured and log of the total fatalities) are the strongest predictors of economic damage. Because the p-values associated with the coefficients are less than our significance level of 0.10, this means that the data suggest that there is a linear relationship between those variables and economic damage. A 1% increase in injuries predicts approximately 0.422% higher economic losses, and a 1% increase in fatalities predicts approximately 0.161% higher losses, holding other variables constant. This aligns with our discussion in the introduction suggesting that casualties serve as a proxy for disaster intensity: more severe events damage infrastructure and harm public health systems, all of which result in greater economic losses. Disaster duration was also a meaningful predictor (coefficient p-value less than 0.10), with a 1% increase in duration predicting a 0.285% increase in expected damage, consistent with the intuition that prolonged events allow for greater cumulative destruction.

Regarding disaster type, wildfires were the only significant level (p = 0.058 \< 0.10), with expected damages roughly 3.25 times higher than earthquakes (the baseline), holding all other predictors constant. While storms and floods are the most frequent disaster types in our data, they are not meaningful predictors of economic damage compared to earthquakes, after adjusting for log of human impact measures and log of duration. 
...
The predictive findings from this analysis have meaningful implications for a lot of of public institutions. Government agencies and emergency management organizations could use an refined version of this model to generate rapid damage forecasts from early casualty estimates, informing how aid and recovery resources are prioritized in the immediate aftermath of a disaster. Infrastructure and city planning institutions could similarly use these findings to guide investments in disaster resilience since duration is a meaningful predictor of economic damage, which could yield great economic benefits.
...
-   World Wildlife Fund. (n.d.). *Is climate change increasing the risk of disasters?* World Wildlife Fund.

    -   <https://www.worldwildlife.org/resources/explainers/is-climate-change-increasing-the-risk-of-disasters/>.
...
-   Centre for Research on the Epidemiology of Disasters (CRED). (n.d.). *EM-DAT public table: Data structure and content.*
...
events |> 
  ...
  geom_histogram() + 
  labs(
    x = "Economic Losses (thousands, USD)",
    title = "Distribution of Economic Damage of Disasters"
  )

```

Summary statistics of log-transformed economic losses:
...
There are no influential points in our dataset.
...
-   I used AI to debug code when errors arose.

-   I used AI when trying to format graphs that were not up to standard.

-   I used AI to find creative visualizations within ggplot package for EDA.

-   I used AI to research past papers/research that were tangential to our project so that we may build/learn from them.
